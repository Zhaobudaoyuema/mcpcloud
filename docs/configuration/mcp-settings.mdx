...
description: 'Configure MCP servers and their settings for MCPCloud'
---

# MCP Settings Configuration

This guide explains how to configure MCP servers in MCPCloud using the `mcp_settings.json` file and related configurations.

## Configuration Files Overview

MCPCloud uses several configuration files:
...
MCPCloud supports environment variable substitution using `${VAR_NAME}` syntax:
...
MCPCloud supports routing STDIO server network traffic through a proxy using **proxychains4**. This feature is available on **Linux and macOS only** (Windows is not supported).
...
MCPCloud supports hot reloading of configurations:
...
MCPCloud validates configurations on startup and reload:
...
This comprehensive guide covers all aspects of configuring MCP servers in MCPCloud for various use cases and environments.
