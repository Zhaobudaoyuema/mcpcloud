    ...
    <div className="page-card mb-4">
      ...
      >
        <div className="flex-1">
          <h3 className="text-lg font-medium text-warm-ink">
            {prompt.name.replace(server + nameSeparator, '')}
            {prompt.title && (
              <span className="ml-2 text-sm font-normal text-warm-warmGray">{prompt.title}</span>
            )}
      ...
      {isExpanded && (
        <div className="mt-4 space-y-4 p-4 pt-0">
          {/* Run Form */}
          {showRunForm && (
            <div className="rounded-warm-card border border-warm-camel/40 bg-warm-cream/70 p-4">
              <DynamicForm
          ...
          {!showRunForm && prompt.arguments && prompt.arguments.length > 0 && (
            <div className="rounded-warm-card border border-warm-camel/40 bg-warm-cream/70 p-3">
              <h4 className="mb-2 text-sm font-medium text-warm-ink">{t('tool.parameters')}</h4>
              <div className="space-y-2">
                {prompt.arguments.map((arg, index) => (
                  <div key={index} className="flex items-start">
                    <div className="flex-1">
                      <div className="flex items-center">
                        <span className="font-medium text-warm-ink">{arg.name}</span>
                        {arg.required && <span className="ml-1 text-red-500">*</span>}
                      </div>
                      {arg.description && (
                        <p className="mt-1 text-sm text-warm-warmGray">{arg.description}</p>
                      )}
                    </div>
                    <div className="ml-2 text-xs text-warm-warmGray">{arg.title || ''}</div>
                  </div>
                ))}
              </div>
            </div>
