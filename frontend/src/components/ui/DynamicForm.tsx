...
const DynamicForm: React.FC<DynamicFormProps> = ({
      ...
      if (schema.enum) {
        return (
          <select
            ...
            className="w-full rounded-md border border-gray-300 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-warm-caramel form-input"
          >
    ...
    if (schema.type === 'boolean') {
      return (
          <input
            ...
            className="h-4 w-4 border-gray-300 text-warm-caramel focus:ring-warm-caramel rounded"
          />
      );
    }
  ...
  const renderField = (key: string, propSchema: JsonSchema, path: string = ''): React.ReactNode => {
    ...
    if (propSchema.type === 'array') {
      ...
      return (
        <div key={fullPath} className="mb-6">
          ...
          <div className="rounded-md border border-gray-200 bg-warm-cream p-3">
            {arrayValue.map((item: any, index: number) => (
              <div key={index} className="mb-3 rounded-md border border-gray-200 bg-warm-cream p-3">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-sm font-medium text-warm-warmGray">
                  ...
                  <button
                    ...
                    className="text-sm text-status-red hover:text-red-700"
                  >
                    {t('common.remove')}
                  </button>
                </div>
                ...
                {propSchema.items?.type === 'string' && propSchema.items.enum ? (
                  <select
                    ...
                    className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-warm-caramel form-input"
                  >
                    <option value="">{t('tool.selectOption')}</option>
                  ...
                  <div className="space-y-3">
                    {Object.entries(propSchema.items.properties).map(([objKey, objSchema]) => (
                      <div key={objKey}>
                        <label className="mb-1 block text-xs font-medium text-warm-warmGray">
                          {objKey}
            ...
            <button
              ...
              className="w-full mt-2 px-3 py-2 text-sm btn-secondary"
            >
              {t('tool.addItem', { key })}
            </button>
          </div>
    ...
    if (propSchema.type === 'object') {
      if (propSchema.properties) {
        ...
        return (
          <div key={fullPath} className="mb-6">
            <label className="mb-1 block text-sm font-medium text-warm-ink">
              {key}
              {(path
                ? getNestedValue(jsonSchema, path)?.required?.includes(key)
                : jsonSchema.required?.includes(key)) && (
            ...
            {propSchema.description && (
              <p className="mb-2 text-xs text-warm-warmGray">{propSchema.description}</p>
            )}

            <div className="rounded-md border border-gray-200 bg-warm-cream p-4">
        ...
        return (
          <div key={fullPath} className="mb-4">
            ...
            <textarea
              ...
              placeholder={`{\n  "key": "value"\n}`}
              className={`w-full rounded-md border px-3 py-2 font-mono text-sm ${error ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-warm-caramel`}
              rows={4}
            />
            {error && <p className="text-status-red text-xs mt-1">{error}</p>}
          </div>
        ...
        return (
          <div key={fullPath} className="mb-4">
            ...
            <select
              ...
              className={`w-full rounded-md border px-3 py-2 ${error ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-warm-caramel form-input`}
            >
              <option value="">{t('tool.selectOption')}</option>
        ...
        return (
          <div key={fullPath} className="mb-4">
            ...
            <input
              ...
              value={value || ''}
              onChange={(e) => handleInputChange(fullPath, e.target.value)}
              className={`w-full rounded-md border px-3 py-2 ${error ? 'border-red' : 'border-gray-200'} focus:outline-none form-input`}
            />
            {error && <p className="text-status-red text-xs mt-1">{error}</p>}
          </div>
        );
      ...
      return (
        <div key={fullPath} className="mb-4">
          ...
          <input
            ...
            className={`w-full rounded-md border px-3 py-2 form-input ${error ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-warm-caramel`}
          />
          {error && <p className="text-status-red text-xs mt-1">{error}</p>}
        </div>
      );
  ...
  if (!jsonSchema.properties) {
    return (
      <div className="rounded-md bg-warm-cream p-4">
        <p className="text-sm text-warm-warmGray">{t('tool.noParameters')}</p>
  ...
  return (
      ...
      <div className="flex items-center justify-between pb-3">
        <h6 className="text-md font-medium text-warm-ink">{title}</h6>
        <div className="flex space-x-2">
          <button
            type="button"
            onClick={switchToFormMode}
            className={`px-3 py-1 text-sm rounded-md transition-colors ${
              !isJsonMode ? 'btn-primary' : 'btn-secondary'
            }`}
          >
            {t('tool.formMode')}
          </button>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder={`{\n  "key": "value"\n}`}
              className={`h-64 w-full resize-y rounded-md border px-3 py-2 font-mono text-sm form-input ${
                jsonError ? 'border-red-500' : 'border-gray-300'
              } focus:outline-none focus:ring-2 focus:ring-warm-caramel`}
            />
