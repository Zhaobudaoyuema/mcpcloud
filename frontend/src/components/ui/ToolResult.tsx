...
const ToolResult: React.FC<ToolResultProps> = ({ result, onClose }) => {
  ...
  const renderContentItem = (item: any): React.ReactNode => {
    if (typeof item === 'string') {
      ...
      return (
        <div className="rounded-warm-card border border-warm-camel/40 bg-warm-cream/70 p-3">
          ...
          <pre className="whitespace-pre-wrap text-sm font-mono text-warm-ink">
            {sanitizedText}
          </pre>
        </div>
      );
    ...
    if (typeof item === 'object' && item !== null) {
      ...
      try {
        ...
        return (
          <div className="rounded-warm-card border border-warm-camel/40 bg-warm-cream/70 p-3">
            <div className="mb-2 text-xs text-warm-warmGray">{t('tool.jsonResponse')}</div>
            <pre className="overflow-auto text-sm text-warm-ink">{JSON.stringify(parsed, null, 2)}</pre>
          </div>
        );
      } catch {
    ...
    return (
      <div className="rounded-warm-card border border-warm-camel/40 bg-warm-cream/70 p-3">
        <pre className="whitespace-pre-wrap text-sm font-mono text-warm-ink">{String(item)}</pre>
      </div>
    );
  };
  ...
  return (
    <div className="page-card">
      <div className="rounded-t-warm-card border-b border-warm-camel/40 bg-warm-cream/80 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={onClose}
              className="text-sm text-warm-warmGray hover:text-warm-ink"
            >
            ✕
          </button>
        </div>
