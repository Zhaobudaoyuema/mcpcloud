...
const Sidebar: React.FC<SidebarProps> = ({ collapsed }) => {
  ...
  // Admin base path for all sidebar links
  ...
  return (
    <aside
      className={`relative flex h-full flex-col bg-warm-cream shadow-warm-sm transition-all duration-300 ease-in-out dark:bg-gray-800 ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
        ...
        <nav className="space-y-1 p-3">
          {menuItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `warm-transition flex items-center rounded-warm-btn px-2.5 py-2 text-sm font-medium ${
                  isActive
                    ? 'bg-warm-terracotta text-warm-cream shadow-warm-sm'
                    : 'text-warm-warmGray hover:bg-warm-beige hover:text-warm-ink dark:text-gray-300 dark:hover:bg-gray-700'
                }`
              }
              end={item.path === adminBase}
            >
      ...
      <div className="bg-warm-cream p-3 dark:bg-gray-800">
