# MCPCloud Development Guide & Agent Instructions

**ALWAYS follow these instructions first and only fallback to additional search and context gathering if the information here is incomplete or found to be in error.**

This document serves as the primary reference for all contributors and AI agents working on `@agentn613/mcpcloud`. It provides comprehensive guidance on code organization, development workflow, and project conventions.

## Project Overview

MCPCloud is a TypeScript/Node.js MCP (Model Context Protocol) server management platform that provides unified access through HTTP endpoints. It serves as a centralized dashboard for managing multiple MCP servers with real-time monitoring, authentication, and flexible routing.
...
MCPCloud supports **JSON file** (default) and **PostgreSQL** storage. Set `USE_DB=true` + `DB_URL` to switch.
