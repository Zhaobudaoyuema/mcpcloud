...
# Build frontend (frontend/dist) and backend (dist/). At runtime a single Node process serves both API and UI.
RUN pnpm frontend:build && pnpm build

COPY entrypoint.sh /usr/local/bin/entrypoint.sh
# Ensure LF line endings (fix "no such file or directory" when file has CRLF)
RUN sed -i 's/\r$//' /usr/local/bin/entrypoint.sh
RUN chmod +x /usr/local/bin/entrypoint.sh

# Single server on 3000: API + frontend UI. Map host 80 to 3000 for UI on port 80.
EXPOSE 3000
