...
export class AppServer {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Initialize OAuth authorization server (for MCPCloud's own OAuth)
  ...
  private findAndServeFrontend(): void {
    ...
    if (this.frontendPath) {
      ...
      this.app.get(rootPath, (_req, res) => {
        res
          .status(404)
          .send('Frontend not found. MCPCloud API is running, but the UI is not available.');
      });
    }
  }
  ...
  start(): void {
    this.server = this.app.listen(this.port, () => {
      ...
      if (this.frontendPath) {
        console.log(`Open http://localhost:${this.port} in your browser to access MCPCloud UI`);
      } else {
        console.log(
          `MCPCloud API is running on http://localhost:${this.port}, but the UI is not available`,
        );
      }
    });
  }
