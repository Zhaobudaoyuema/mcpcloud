...
export const initMiddlewares = (app: express.Application): void => {
  ...
  // Protect API routes with authentication middleware, but exclude auth and public read-only endpoints
  app.use(`${config.basePath}/api`, (req, res, next) => {
    ...
    // Allow unauthenticated GET for public-facing pages: servers list, market, cloud, registry
    if (req.method === 'GET') {
      if (
        req.path === '/servers' ||
        /^\/servers\/[^/]+$/.test(req.path) ||
        req.path.startsWith('/market/') ||
        req.path.startsWith('/cloud/') ||
        req.path.startsWith('/registry/')
      ) {
        next();
        return;
      }
